...
def _content_item_has_image(item: Dict) -> bool:
    """判断单个内容块是否包含图片"""
...
def message_has_image(message: Dict) -> bool:
    ...
    if isinstance(content, list):
        for item in content:
            if _content_item_has_image(item):
                return True
    return False


def find_last_image_index(messages: List[Dict]) -> int:
    """找到最近一条包含图片的消息索引"""
    for idx in range(len(messages) - 1, -1, -1):
        if message_has_image(messages[idx]):
            return idx
    return -1
...
async def compress_context(
    ...
    last_image_index = find_last_image_index(messages)
    if last_image_index >= 0 and last_image_index < keep_start:
        logger.info(f"检测到图片消息在索引 {last_image_index}，扩展保留边界避免丢失图片上下文")
        keep_start = last_image_index
    if keep_start <= 0:
